...
function cargarDatos(){
    ...
    eventoBusqueda(listaHerramientas)
    })
}
...
botonvaciar.addEventListener("click", vaciarCarrito)
 ...
 function eventoBusqueda (listaHerramientas){
    let searchBar = document.getElementById("input_text")
    searchBar.addEventListener('keyup', (e) => {
        const textoBusqueda = e.target.value.toLowerCase();
        let herramientasFiltradas = listaHerramientas.filter((herramienta) =>{
            return herramienta.nombre.toLowerCase().includes(textoBusqueda)
        })
